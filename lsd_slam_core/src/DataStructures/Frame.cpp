...
Frame::Frame(int id,
             ...
             const int height_rollingShutter_inputImage)
{
    ...
    // Copy image mask
    data.imageMask[0] = FrameMemory::getInstance().getFloatBuffer(data.width[0]*data.height[0]);
    maxPt = data.imageMask[0] + data.width[0]*data.height[0];
             ...
             const int height_rollingShutter_inputImage)
{
    ...
    // Copy image mask
    data.imageMask[0] = FrameMemory::getInstance().getFloatBuffer(data.width[0]*data.height[0]);
    memcpy(data.imageMask[0], imageMask, data.width[0]*data.height[0] * sizeof(float));
    data.imageMaskValid[0] = true;

}
    
void Frame::setGTMotion()
{
        
    // Set RT_motion if file is available
    if (FILE *fid = fopen(motionFilename.c_str(), "r"))
    {
        ...
        float r31, r32, r33;
        float t1, t2, t3;
        while (fscanf(fid, "%f %f %f %f %f %f %f %f %f %f %f %f\n",
                      &r11, &r21, &r31,
                      &r12, &r22, &r32,
                      &r13, &r23, &r33,
                      &t1, &t2, &t3) != EOF)
        {
            // Assign
            Eigen::Matrix<double,4,4> poseFromFile;
            poseFromFile << r11, r12, r13, t1,
            r21, r22, r23, t2,
            r31, r32, r33, t3,
            0,   0,   0,  1;

            RT_motion.push_back(poseFromFile);
            
        }
        fclose(fid);
        if (RT_motion.size() > 0)
        {
            is_RT_motion_Available = true;
        }
...
Frame::~Frame()
{
	...
	for (int level = 0; level < PYRAMID_LEVELS; ++ level)
	{
        ...
        FrameMemory::getInstance().returnBuffer(data.imageMask[level]);
...
#if 1
// Copy constructor (Jae-Hak Kim, 2015-04-20)
Frame::Frame(const Frame &copyOfFrame)
{

    startRowImage = copyOfFrame.startRowImage;
    startRowSeg = copyOfFrame.startRowSeg;
    rowPerSegment = copyOfFrame.rowPerSegment;
    isSplineValid = copyOfFrame.isSplineValid;
    height_rollingShutter_inputImage =
        copyOfFrame.height_rollingShutter_inputImage;
    
    depthFilename = copyOfFrame.depthFilename;
    imageFilename = copyOfFrame.imageFilename;
    motionFilename = copyOfFrame.motionFilename;
    RT_motion = copyOfFrame.RT_motion;
    is_RT_motion_Available = copyOfFrame.is_RT_motion_Available;
    spline_ = copyOfFrame.spline_;
    undistorter_ = copyOfFrame.undistorter_;

    data.id = copyOfFrame.data.id;

    pose = new FramePoseStruct(*(copyOfFrame.pose));
    //*pose = *(copyOfFrame.pose); // Check copy assignment operator of FramePoseStructure

    for (int i = 0; i<PYRAMID_LEVELS; i++)
    {

        data.width[i] = copyOfFrame.data.width[i];
        data.height[i] = copyOfFrame.data.height[i];

        data.K[i] = copyOfFrame.data.K[i];
        data.fx[i] = copyOfFrame.data.fx[i];
        data.fy[i] = copyOfFrame.data.fy[i];
        data.cx[i] = copyOfFrame.data.cx[i];
        data.cy[i] = copyOfFrame.data.cy[i];

        data.KInv[i] = copyOfFrame.data.KInv[i];
        data.fxInv[i] = copyOfFrame.data.fxInv[i];
        data.fyInv[i] = copyOfFrame.data.fyInv[i];
        data.cxInv[i] = copyOfFrame.data.cxInv[i];
        data.cyInv[i] = copyOfFrame.data.cyInv[i];

    }

    data.timestamp = copyOfFrame.data.timestamp;

    data.hasIDepthBeenSet = copyOfFrame.data.hasIDepthBeenSet;
    this->depthHasBeenUpdatedFlag = copyOfFrame.depthHasBeenUpdatedFlag;

    referenceID = copyOfFrame.referenceID;
    referenceLevel = copyOfFrame.referenceLevel;

    numMappablePixels = copyOfFrame.numMappablePixels;



    data.reActivationDataValid = copyOfFrame.data.reActivationDataValid;
    data.validity_reAct = copyOfFrame.data.validity_reAct;
    data.idepthVar_reAct = copyOfFrame.data.idepthVar_reAct;
    data.idepth_reAct = copyOfFrame.data.idepth_reAct;

    data.refPixelWasGood = copyOfFrame.data.refPixelWasGood;

    permaRefNumPts = copyOfFrame.permaRefNumPts;
    permaRef_colorAndVarData = copyOfFrame.permaRef_colorAndVarData;
    permaRef_posData = copyOfFrame.permaRef_posData;

    meanIdepth = copyOfFrame.meanIdepth;
    numPoints = copyOfFrame.numPoints;

    numFramesTrackedOnThis = copyOfFrame.numFramesTrackedOnThis;
    numMappedOnThis = copyOfFrame.numMappedOnThis;
    numMappedOnThisTotal = copyOfFrame.numMappedOnThisTotal;

    idxInKeyframes = copyOfFrame.idxInKeyframes;

    edgeErrorSum = copyOfFrame.edgeErrorSum;
    edgesNum = copyOfFrame.edgesNum;

    lastConstraintTrackedCamToWorld =
            copyOfFrame.lastConstraintTrackedCamToWorld;

    isActive = copyOfFrame.isActive;

    // Init valid flags and data pointers
    for (int level=0;level<PYRAMID_LEVELS;level++)
    {

        data.imageValid[level] = false;
        data.imageMaskValid[level] = false;
    ...
    // Build image for level 0
    data.image[0] = FrameMemory::getInstance().getFloatBuffer(
                data.width[0]*data.height[0]);
    memcpy(data.image[0], copyOfFrame.data.image[0],
           data.width[0]*data.height[0] * sizeof(float));
    data.imageValid[0] = copyOfFrame.data.imageValid[0];
    
    // Build image mask for level 0
    data.imageMask[0] = FrameMemory::getInstance().getFloatBuffer(
                data.width[0]*data.height[0]);
    memcpy(data.imageMask[0], copyOfFrame.data.imageMask[0],
           data.width[0]*data.height[0] * sizeof(float));
    data.imageMaskValid[0] = copyOfFrame.data.imageMaskValid[0];

    // Build images, gradients, max gradients and inverse depth/var if they
    // are valid in the copy
    for (int level=0; level<PYRAMID_LEVELS; level++)
    {

        if (copyOfFrame.data.imageValid[level] == true)
        {
            if (level >= 1)
            {
                buildImage(level);
        ...
        if (copyOfFrame.data.imageMaskValid[level] == true)
        {
            if (level >= 1)
            {
                buildImageMask(level);
            }
        ...
        if (copyOfFrame.data.gradientsValid[level] == true)
        {
        ...
        if (copyOfFrame.data.maxGradientsValid[level] == true)
        {
        ...
        if (copyOfFrame.data.idepthValid[level] == true)
        {

            //buildIDepthAndIDepthVar(level); // This does not work for level 0
            ...
            for(int y=0;y<height;y++)
            {
                for (int x=0;x<width;x++)
                {
                    ...
                    int idx = x + y*width;
                    data.idepth[level][idx] =
                            copyOfFrame.data.idepth[level][idx];
        ...
        else
        {
            ...
            data.idepthValid[level] = copyOfFrame.data.idepthValid[level];

        }

        if  (copyOfFrame.data.idepthVarValid[level] == true)
        {
            ...
            for(int y=0;y<height;y++)
            {
                for (int x=0;x<width;x++)
                {
                    ...
                    int idx = x + y*width;
                    data.idepthVar[level][idx] =
                            copyOfFrame.data.idepthVar[level][idx];
        ...
        else
        {
            ...
            data.idepthVarValid[level] = copyOfFrame.data.idepthVarValid[level];
    ...
    referenceID = copyOfFrame.referenceID;
    referenceLevel = copyOfFrame.referenceLevel;
    distSquared = copyOfFrame.distSquared;
    K_otherToThis_R = copyOfFrame.K_otherToThis_R;
    K_otherToThis_t = copyOfFrame.K_otherToThis_t;
    otherToThis_t = copyOfFrame.otherToThis_t;
    K_thisToOther_t = copyOfFrame.K_thisToOther_t;
    thisToOther_R = copyOfFrame.thisToOther_R;
    otherToThis_R_row0 = copyOfFrame.otherToThis_R_row0;
    otherToThis_R_row1 = copyOfFrame.otherToThis_R_row1;
    otherToThis_R_row2 = copyOfFrame.otherToThis_R_row2;
    thisToOther_t = copyOfFrame.thisToOther_t;

    // Statistics
    initialTrackedResidual = copyOfFrame.initialTrackedResidual;
    numFramesTrackedOnThis = copyOfFrame.numFramesTrackedOnThis;
    numMappedOnThis = copyOfFrame.numMappedOnThis;
    numMappedOnThisTotal = copyOfFrame.numMappedOnThisTotal;
    meanIdepth = copyOfFrame.meanIdepth;
    ...
    edgesNum = copyOfFrame.edgesNum;
    numMappablePixels = copyOfFrame.numMappablePixels;
    meanInformation = copyOfFrame.meanInformation;

    isActive = copyOfFrame.isActive;
    
    // Copy filenames
    depthFilename = copyOfFrame.depthFilename;
    imageFilename = copyOfFrame.depthFilename;
    motionFilename = copyOfFrame.motionFilename;
    RT_motion = copyOfFrame.RT_motion;
    is_RT_motion_Available = copyOfFrame.is_RT_motion_Available;
...
// Prepare rolling shutter stereo with frame at pixel
void Frame::prepareForRollingShutterStereo(
            const Frame* other,
            Sim3 thisToOther,
            const int y, // Row of pixel in "Other" frame
            const Eigen::Matrix3f& K,
            const int level)
{
    // Here, thisToOther means this frame with respect to other keyframe.
    //
    // Note: Our assumption is that the first row pose is a repsentative pose
    //       of the rolling shutter frame.
    //       Therefore, for each row, we provide each different otherToThis pose
    //       for rolling shutter stereo.
    //
    // "this (Frame)" is TrackingFrame (Target image in warping)
    // "other" is TrackingReference (Source image in warping)

#if 1
    
    //--------------------------------------------------------------------------
    // Get a pose of the row and apply to thisToOther transformation
    
    Eigen::Matrix<double,4,4> worldToRowPose = getPoseAtRow(y, level);
    Sophus::Sim3d Xi_worldToRowPose =
        sim3FromSE3(Sophus::SE3d(worldToRowPose), 1.0);
    
    // Note: thisToOther is referenceToKeyFrame. i.e. FirstRowPoseToRef.
    //
    // Then, FirstRowPoseToRef
    //       = FrameToRef * FirstRowPoseToFrame
    //       = FrameToRef * (WorldToFrame * FirstRowPoseToWorld)
    //       = thisToOther * (WorldToThis * FirstRowPoseToWorld)
    //
    // Apply thisToOther for a row of pixel
    thisToOther = thisToOther * (this->pose->getCamToWorld().inverse()
                                 * Xi_worldToRowPose.inverse());
    //--------------------------------------------------------------------------
...
void Frame::require(int dataFlags, int level)
{
    ...
    if ((dataFlags & IMAGE_MASK) && ! data.imageMaskValid[level])
    {
        buildImageMask(level);
    }
...
{
    // Set row per segment for rolling shutter
    this->height_rollingShutter_inputImage = height_rollingShutter_inputImage;
...
void Frame::buildImageMask(int level)
{
    if (level == 0)
    {
        printf("Frame::buildImageMask(0): Loading image from disk is not implemented yet! No-op.\n");
        return;
    }
    
    require(IMAGE_MASK, level - 1);
    boost::unique_lock<boost::mutex> lock2(buildMutex);
    ...
    if(enablePrintDebugInfo && printFrameBuildDebugInfo)
        printf("CREATE ImageMask lvl %d for frame %d\n", level, id());
    
    int width = data.width[level - 1];
    int height = data.height[level - 1];
    const float* source = data.imageMask[level - 1];
    
    if (data.imageMask[level] == 0)
        data.imageMask[level] = FrameMemory::getInstance().getFloatBuffer(data.width[level] * data.height[level]);
    float* dest = data.imageMask[level];
    ...
    if (width % 8 == 0)
    {
        ...
        data.imageMaskValid[level] = true;
        return;
    }
#elif defined(ENABLE_NEON)
    // I assume all all subsampled width's are a multiple of 8.
...
void Frame::releaseImageMask(int level)
{
    if (level == 0)
    {
        printf("Frame::releaseImageMask(0): Storing image on disk is not supported yet! No-op.\n");
        return;
    }
    FrameMemory::getInstance().returnBuffer(data.image[level]);
    data.imageMask[level] = 0;
...
{
	require(IMAGE, level);
    require(IMAGE_MASK, level); // Image mask also needed
	boost::unique_lock<boost::mutex> lock2(buildMutex);
...
#if 1 // (ORIGINAL) Prewitt operator
    ...
    const float* mask_pt = data.imageMask[level] + width;
	...
	for(; img_pt < img_pt_max; img_pt++, gradxyii_pt++, mask_pt++)
	{

        if (*mask_pt != 0) // Valid mask 255, Invalid mask 0
        {
            
            // Compute gradient only when mask is valid
            val_p1 = *(img_pt+1);
    ...
    const float* img_pt_max = data.image[level] + width*(height - 1) - 2;
    Eigen::Vector4f* gradxyii_pt = data.gradients[level] + width + 1;
    
    // in each iteration i need -1,0,p1,mw,pw
    // val_m0 val_00 val_p0
    // val_m1 val_01 val_p1
    // val_m2 val_02 val_p2
    // where val_01 has image point center
    float val_m0 = *(img_pt - width - 1);
    float val_m1 = *(img_pt - 1);
    float val_m2 = *(img_pt + width - 1);
    float val_00 = *(img_pt - width);
    float val_01 = *img_pt;
    float val_02 = *(img_pt + width);
    float val_p0, val_p1, val_p2;
    
    for(; img_pt < img_pt_max; img_pt++, gradxyii_pt++)
    {
        val_p0 = *(img_pt - width + 1);
        val_p1 = *(img_pt + 1);
        val_p2 = *(img_pt + width + 1);
        
        // Gradient in x
        *((float*)gradxyii_pt) = 0.125f*((val_p0 - val_m0)
                                         + 2.0f*(val_p1 - val_m1)
                                         + (val_p2 - val_m2));
        // Gradient in y
        *(((float*)gradxyii_pt)+1) = 0.125f*((val_p2 - val_p0)
                                             + 2.0f*(val_02 - val_00)
                                             + (val_m2 - val_m0));
        // Intensity
        *(((float*)gradxyii_pt)+2) = val_01;
