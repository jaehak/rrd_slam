...
void SlamSystem::debugDisplayDepthMap()
{
        ...
        (lsd_slam::plotTrackingIterationInfo == true))
    {
        ...
        // Lock ensuring to save files
        //currentKeyFrameMutex.lock();

        char buf[255];
        if (tracker->getFrameNumberToDisplay() >= 0)
        {
            
            sprintf(buf, "depth_%06d.png", tracker->getFrameNumberToDisplay());
            cv::imwrite(buf, map->debugImageDepth);
        
#if 1 // SAVE depth as file
        sprintf(buf, "depth_%06d.txt", tracker->getFrameNumberToDisplay());
        if (!isFileExist(buf))
...
void SlamSystem::gtDepthInitWithName(uchar* image,
                                     float* depth,
                                     double timeStamp,
                                     int id,
...
#else // Modified by Jae-Hak Kim, 2015-04-16
        SE3 c2f_init = se3FromSim3(candidateToFrame_initialEstimateMap[candidate]);
#endif
...
std::vector<Frame*> SlamSystem::getKeyframesAll()
{
    return keyFrameGraph->keyframesAll;
}
