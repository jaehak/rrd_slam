...
// Copy constructor (written by Jae-Hak Kim, 2015-04-20)
TrackingReference::TrackingReference(
        const TrackingReference &copyOfTrackingReference)
{

    // Copy keyframe and frameID
    this->importFrame(copyOfTrackingReference.keyframe);

    //this->keyframe = new Frame(*(copyOfTrackingReference.keyframe));
    ...
    for (int level=0;level<PYRAMID_LEVELS;level++)
    {
        ...
        // Allocate data
        int w = copyOfTrackingReference.keyframe->width(level);
        int h = copyOfTrackingReference.keyframe->height(level);
        if(posData[level] == nullptr) posData[level] = new Eigen::Vector3f[w*h];
        if(pointPosInXYGrid[level] == nullptr) pointPosInXYGrid[level] = new int[w*h];
        if(gradData[level] == nullptr) gradData[level] = new Eigen::Vector2f[w*h];
        if(colorAndVarData[level] == nullptr) colorAndVarData[level] = new Eigen::Vector2f[w*h];
        ...
        Eigen::Vector3f* copy_posDataPT =
                copyOfTrackingReference.posData[level];
        int *copy_idxPT = copyOfTrackingReference.pointPosInXYGrid[level];
        Eigen::Vector2f* copy_gradDataPT =
                copyOfTrackingReference.gradData[level];
        Eigen::Vector2f* copy_colorAndVarDataPT =
                copyOfTrackingReference.colorAndVarData[level];

        // Copy data
        for (int i=0;i<copyOfTrackingReference.numData[level];i++)
        {
        ...
        numData[level] = copyOfTrackingReference.numData[level];
        wh_allocated = copyOfTrackingReference.wh_allocated;
