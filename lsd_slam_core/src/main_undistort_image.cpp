...
int main( int argc, char** argv )
{
...
#if 0 // RGB
    int IMAGE_TYPE = CV_8UC3; // RGB
    int LOAD_IMAGE_TYPE = CV_LOAD_IMAGE_COLOR; // COLOR
#else
    int IMAGE_TYPE = CV_8UC1; // GRAY
    int LOAD_IMAGE_TYPE = CV_LOAD_IMAGE_GRAYSCALE; // GRAY
#endif

    cv::Mat image = cv::Mat(h,w,IMAGE_TYPE);
    cv::Mat image_redistort = cv::Mat(h,w,IMAGE_TYPE);
	int runningIDX=0;
	...
	for(unsigned int i=0;i<files.size();i++)
	{
        cv::Mat imageDist = cv::imread(files[i], LOAD_IMAGE_TYPE);
        ...
        assert(imageDist.type() == IMAGE_TYPE);

#if 0 // RGB
        undistorter->undistortRGB(imageDist, image);
#else // GRAY
        undistorter->undistort(imageDist, image);
#endif
        
        assert(image.type() == IMAGE_TYPE);
...
*/
        ...
        char buf[255];
        sprintf(buf, "%06d.png", runningIDX + 1); // 1-based index
        printf("Saving: %s\n", buf);
        cv::imwrite(buf, image);
        
//        // Save re-distorted image
//        cv::imshow("image_redistort", image_redistort);
//        sprintf(buf, "%06d_redistort.png", runningIDX + 1);
//        prinrtf("Saving: %s\n", buf);
//        cv::imwrite(buf, image_redistort);
...
*/
    ...
    std::cout << "New K = " << std::endl;
    std::cout << undistorter->getK() << std::endl;
