...
public:
    Frame *getActiveKeyFrame() { return activeKeyFrame; }
    void setActiveKeyFrame(Frame *frame)
    {
	...
	virtual void initializeFromGTDepth(Frame* new_frame);
    ...
    public: void makeMeanInvDepthOne();
    
    // Initialize from keyframe
    public: void initializeFromKeyframe(Frame* keyframe);
...
protected:
    ...
    // GT depth for keyframes
    void setDepthFromGT();
    void setDepthFromGTRow(int yMin, int yMax, RunningStats* stats);
    std::shared_ptr<float> readDepthFile(std::string filename, bool *status);
