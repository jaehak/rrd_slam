                                  ...
                                  float maxInitDepth)
{
	...
	for(int y=1;y<height-1;y++)
	{
		for(int x=1;x<width-1;x++)
		{
			...
			{
#if 0 // ORIGINAL depth range [0.66m ~ 2m] => idepth [1.5 ~ 0.5]
				float idepth = 0.5f + 1.0f * ((rand() % 100001) / 100000.0f);
#else // WIDER RANDOM INIT range (0.65 ~ 4.0)
                //float idepth = 0.25f + 0.4f * ((rand() % 100001) / 100000.0f);

                // Range depth [1.5m ~ 4m] => idepth [0.25 ~ 0.65]
                //float idepth = 0.25f + 0.4f * ((rand() % 100001) / 100000.0f);
                
                // Much wider range [0.9m ~ 10m] => idepth [0.1 ~ 1.0]
                //float idepth = 0.1f + 1.0f*((rand() % 100001) / 100000.0f);
                float idepth = 1.0f/maxInitDepth +
                               (1.0f/minInitDepth - 1.0f/maxInitDepth)*
...
void DepthMap::initializeFromKeyframe(Frame* keyframe)
{
    
    assert(keyframe->hasIDepthBeenSet());
    
    activeKeyFramelock = keyframe->getActiveLock();
    activeKeyFrame = keyframe;
    activeKeyFrameImageData = activeKeyFrame->image(0);
    activeKeyFrameIsReactivated = false;
    
    const float* maxGradients = keyframe->maxGradients();
    ...
    for(int y=1;y<height-1;y++)
    {
        for(int x=1;x<width-1;x++)
        {
            if(maxGradients[x+y*width] > MIN_ABS_GRAD_CREATE)
            {
                ...
                const float *idepth = keyframe->idepth(0) + x + y*width;
                const float *idepthVar = keyframe->idepthVar(0) + x + y*width;
                currentDepthMap[x+y*width] =
                    DepthMapPixelHypothesis(*idepth,
                                            *idepth,
                                            *idepthVar,
                                            *idepthVar,
                                            20);
...
void DepthMap::setDepthFromGTRow(int yMin, int yMax, RunningStats* stats)
{
    ...
    if (activeKeyFrame != 0)
    {
        ...
        bool isReadWell = false;
        std::shared_ptr<float> depthGT = readDepthFile(filename, &isReadWell);
        
        if (isReadWell == false)
        {
            return; // No depth file exist
        }
...
//
void DepthMap::saveDepthAsFile(char *filename)
{
