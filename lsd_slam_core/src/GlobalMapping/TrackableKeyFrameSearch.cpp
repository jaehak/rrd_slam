...
#else // Modified by Jae-Hak Kim, 2015-04-16 TESTING...

        // Inverse of Sim3 not recommended but instead do the inverse in SE3 before multiplcation
        potentialReferenceFrames.back().refToFrame =
          se3FromSim3(graph->keyframesAll[i]->getScaledCamToWorld()).inverse()
          * se3FromSim3(frame->getScaledCamToWorld()).inverse();
