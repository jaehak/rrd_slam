...
cv::Vec3b DepthMapPixelHypothesis::getVisualizationColor(int lastFrameID) const
{
	if(debugDisplay == 0 || debugDisplay == 1)
	{
...
#if 1 // ORIGINAL RAINBOW
		// rainbow between 0 and 4
		float r = (0-id) * 255 / 1.0; if(r < 0) r = -r;
        float g = (1-id) * 255 / 1.0; if(g < 0) g = -g;
		float b = (2-id) * 255 / 1.0; if(b < 0) b = -b;
#else
        // rainbow between 0 and 5
        float r = (0.0-id) * 255 / 0.4; if(r < 0) r = -r;
        float g = (0.4-id) * 255 / 0.4; if(g < 0) g = -g;
        float b = (0.8-id) * 255 / 0.4; if(b < 0) b = -b;
#endif
        ...
        // Middle: Green
        // Far: Blue
        // Check the RGB space of your image storing this pixel value
        // In case, BGR space this close and far information could be reversed
        // Note: Jae-Hak Kim, 2015-09-14
